...
class SingleFragmentEmbedding(DMET):
    ...
    using density-matrix-diagonalization based DMET in non-orthogonal AO basis.

    This class performs a single-shot, single-fragment exact subspace
    energy evaluation with hybrid XC functional treatment.
    """
    ...
    def __init__(self, mf_outer, mf_inner, fragment, threshold=1e-2, verbose=None):
        ...
        threshold : float
            Electron-number tolerance (default 1e-2) for orbital selection.
        """
        fragments = [fragment]

...
def grad_elec(td_grad, x_y, singlet=True, atmlst=None,
    ...
    import pdb
    pdb.set_trace()
    dvv = numpy.einsum('ai,bi->ab', xpy, xpy) + numpy.einsum('ai,bi->ab', xmy, xmy) # T_{ab} 2 for alpha and beta
    doo =-numpy.einsum('ai,aj->ij', xpy, xpy) - numpy.einsum('ai,aj->ij', xmy, xmy) # T_{ij}
    dmxpy = reduce(numpy.dot, (orbv, xpy, orbo.T)) # (X+Y) in ao basis
    dmxmy = reduce(numpy.dot, (orbv, xmy, orbo.T)) # (X-Y) in ao basis
    dmzoo = reduce(numpy.dot, (orbo, doo, orbo.T)) # T_{ij}*2 in ao basis
    dmzoo+= reduce(numpy.dot, (orbv, dvv, orbv.T)) # T_{ij}*2 + T_{ab}*2 in ao basis

    vj, vk = mf.get_jk(mol, (dmzoo, dmxpy+dmxpy.T, dmxmy-dmxmy.T), hermi=0)
    veff0doo = vj[0] * 2 - vk[0] #  H_{uv}^+[T] 2 for alpha and beta
    wvo = reduce(numpy.dot, (orbv.T, veff0doo, orbo)) * 2 # H_{ai}+[T]
    if singlet:
        veff = vj[1] * 2 - vk[1] # H_{uv}^+[(X+Y)]
    else:
        veff = -vk[1] #  H_{uv}^+[(X+Y)]
    veff0mop = reduce(numpy.dot, (mo_coeff.T, veff, mo_coeff)) #  H_{pq}^+[(X+Y)]
    wvo -= numpy.einsum('ki,ai->ak', veff0mop[:nocc,:nocc], xpy) * 2 # H_{ai}+[T] - sum_{j} (X+Y)_{aj}H_{ij}+[(X+Y)]
    # H_{ai}+[T] - sum_{j} (X+Y)_{aj}H_{ij}+[(X+Y)] + sum_{b} (X+Y)_{bi}H_{ab}+[(X+Y)]
    # 2 for dm + dm.T
    wvo += numpy.einsum('ac,ai->ci', veff0mop[nocc:,nocc:], xpy) * 2 
    veff = -vk[2] #H_{uv}^-[(X-Y)]
    veff0mom = reduce(numpy.dot, (mo_coeff.T, veff, mo_coeff)) # H_{pq}^-[(X-Y)]
    # H_{ai}+[T] - sum_{j} (X+Y)_{aj}H_{ij}+[(X+Y)] + sum_{b} (X+Y)_{bi}H_{ab}+[(X+Y)] - sum_{j} (X-Y)_{aj}H_{ij}+[(X-Y)]
    wvo -= numpy.einsum('ki,ai->ak', veff0mom[:nocc,:nocc], xmy) * 2
    # H_{ai}+[T] - sum_{j} (X+Y)_{aj}H_{ij}+[(X+Y)] + sum_{b} (X+Y)_{bi}H_{ab}+[(X+Y)] - sum_{j} (X-Y)_{aj}H_{ij}^-[(X-Y)] + sum_{b} (X-Y)_{bi}H_{ab}^-[(X-Y)]
    ...
    def fvind(x):  # For singlet, closed shell ground state
        dm = reduce(numpy.dot, (orbv, x.reshape(nvir,nocc)*2, orbo.T)) # 2 for double occupancy
    ...
    veff = vresp(z1ao+z1ao.T) # H_{uv}^+[Z] # this is intented to input dm + dm.T, the answer is based on dm scale.

    im0 = numpy.zeros((nmo,nmo))
    # in the following, all should be doubled, due to double occupancy
    # and 0.5 for i<=j and a<= b
    # but this is reduced.
    im0[:nocc,:nocc] = reduce(numpy.dot, (orbo.T, veff0doo+veff, orbo)) # H_{ij}^+[T] + H_{ij}^+[Z] # 
    im0[:nocc,:nocc]+= numpy.einsum('ak,ai->ki', veff0mop[nocc:,:nocc], xpy) # H_{ij}^+[T] + H_{ij}^+[Z] + sum_{a} (X+Y)_{aj}H_{ai}^+[(X+Y)]
    im0[:nocc,:nocc]+= numpy.einsum('ak,ai->ki', veff0mom[nocc:,:nocc], xmy) # H_{ij}^+[T] + H_{ij}^+[Z] + sum_{a} (X+Y)_{aj}H_{ai}^+[(X+Y)] + sum_{a} (X-Y)_{aj}H_{ai}^-[(X-Y)]
    im0[nocc:,nocc:] = numpy.einsum('ci,ai->ac', veff0mop[nocc:,:nocc], xpy) #  sum_{i} (X+Y)_{ci}H_{ai}^+[(X+Y)]
    im0[nocc:,nocc:]+= numpy.einsum('ci,ai->ac', veff0mom[nocc:,:nocc], xmy) #  sum_{i} (X+Y)_{ci}H_{ai}^+[(X+Y)] + sum_{i} (X-Y)_{cj}H_{ai}^-[(X-Y)]
    im0[nocc:,:nocc] = numpy.einsum('ki,ai->ak', veff0mop[:nocc,:nocc], xpy)*2 #  sum_{i} (X+Y)_{ki}H_{ai}^+[(X+Y)] * 2
    im0[nocc:,:nocc]+= numpy.einsum('ki,ai->ak', veff0mom[:nocc,:nocc], xmy)*2 #  sum_{i} (X+Y)_{ki}H_{ai}^+[(X+Y)] + sum_{i} (X-Y)_{ki}H_{ai}^-[(X-Y)] * 2

    zeta = lib.direct_sum('i+j->ij', mo_energy, mo_energy) * .5 # E_{pq} = (E_p + E_q) / 2
    zeta[nocc:,:nocc] = mo_energy[:nocc] # E_{ai} = E_i
    zeta[:nocc,nocc:] = mo_energy[nocc:] # E_{ia} = E_a
    dm1 = numpy.zeros((nmo,nmo))
    dm1[:nocc,:nocc] = doo # T_{ij}*2
    dm1[nocc:,nocc:] = dvv # T_{ab}*2
    dm1[nocc:,:nocc] = z1
    dm1[:nocc,:nocc] += numpy.eye(nocc)*2 # for ground state
    im0 = reduce(numpy.dot, (mo_coeff, im0+zeta*dm1, mo_coeff.T)) # W_{\mu\nu} ?
    ...
    oo0 = reduce(numpy.dot, (orbo, orbo.T)) #D
    ...
    for k, ia in enumerate(atmlst):
        ...
        # doublel occupancy
        de[k] = numpy.einsum('xpq,pq->x', h1ao, oo0) * 2 # ground state.

                 ...
                 )
{
    ...
    for (int kl_bin = 0; kl_bin < nbins_kl; kl_bin++) {
        ...
        if (ntasks_ij_single > 0) {
            ...
            // const int err = GINTrun_tasks_jk<double, double, double, double>(&jk, &offsets, &envs);
    ...
    // printf("Henry debug GINTbuild_jk i = %d, j = %d, k = %d, l = %d, ij_nprim = %d, kl_nprim = %d\n", cp_ij->l_bra, cp_ij->l_ket, cp_kl->l_bra, cp_kl->l_ket, cp_ij->nprim_12, cp_kl->nprim_12); fflush(stdout);
    // printf("Henry debug GINTbuild_jk n_tasks_count_double = %d, n_tasks_count_single = %d\n", n_tasks_count_double, n_tasks_count_single); fflush(stdout);
